describe('Login page saucedemo test case', () => {
	before('Visit the website SwagLabs', () => {
		cy.visit('/')
	...
	it('Should try to login value null', () => {
		cy.fixture('usersaucedemo').then(user => {
			...
			// Error message
			cy.get("div[class='error-message-container error']")
				.should('be.visible')
				.and(
					'contain',
					'Epic sadface: Username and password do not match any user in this service'
				)
			// Clicking error message
	...
	it('Should try to selecting product add to cart ', () => {
		cy.visit('/')
		cy.url().should('include', 'https://www.saucedemo.com/')
